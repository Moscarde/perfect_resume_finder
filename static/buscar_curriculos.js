    ...
    .then(data => {
        ...
        if (data.length > 0) {
            ...
            data.forEach(item => {
                ...
                resumeUrlCell.innerHTML = '<a href="' + item.resume_url + '" target="_blank">Ver Curriculo</a>';
            ...
            cell.colSpan = 4; // Colspan para abranger todas as colunas
...
document.querySelectorAll('.check-btn').forEach(checkBtn => {
    checkBtn.addEventListener('change', function() {
        ...
        inputField.value = currentValue.join(', '); // Atualiza o campo de entrada
    });
});
